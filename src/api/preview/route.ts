...
async function imageToDataUri(url: string): Promise<{ dataUrl: string, mimeType: string }> {
    try {
        ...
        const reader = new FileReader(); // This is not available on server, need a different approach
        // Instead, use Buffer for server-side conversion
    ...
    const resolvedOverlays: ImageTransform[] = await Promise.all(
        body.overlays.map(async (item): Promise<ImageTransform> => {
            ...
            return {
                ...
                width: (item.itemType === 'image' || item.itemType === 'text') ? (100 * (item.scale || 1)) : (item.width! * (item.scale || 1)),
                height: (item.itemType === 'image' || item.itemType === 'text') ? (100 * (item.scale || 1)) : (item.height! * (item.scale || 1)),
