...
function PrivacyPolicyPageLoadingSkeleton() {
    return (
...
export default function PrivacyPolicyPage() {
    ...
    if (isLoading || !storeConfig) {
        return <PrivacyPolicyPageLoadingSkeleton />;
    }
    ...
    const pageContent = storeConfig.pages?.privacy;
    const hasContent = pageContent?.body && pageContent.body.trim() !== '';
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background">
            ...
            <main className="flex-1 py-12 md:py-16">
                <div className="container max-w-3xl mx-auto px-4">
                    <h1 className="text-4xl font-bold font-headline text-foreground mb-8">{pageContent?.title || 'Privacy Policy'}</h1>
                    <div className="prose prose-lg dark:prose-invert max-w-none text-muted-foreground">
                        {hasContent ? (
                            <div dangerouslySetInnerHTML={{ __html: pageContent.body.replace(/\n/g, '<br />') }} />
                        ) : (
                            <p>A Privacy Policy has not been configured for this store. This content can be edited in the store dashboard.</p>
                        )}
                    </div>
                </div>
            </main>
